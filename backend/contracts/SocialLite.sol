...
contract Social{
    ...
    function getMessage(string calldata _information)public{
        ...
        messages[index] = Message(msg.sender,_information,index,numberOfMessages,0,0);
...
numberOfMessages = numberOfMessages.add(1);
    ...
    function writeComment(uint _index,string calldata _comment)public{
        ...
        commentsUnderMessage[_index][commentNumber] = Comment(msg.sender,_comment);
...
for(uint index =0 ; index < numberOfMessages; index++){
    ...
    function readComments(uint _index)public view returns (Comment[] memory comment){
...
for(uint index =0 ; index < numberOfComments; index++){
    ...
    function readindividualMessage()public view returns (Message[] memory message){
                  uint usermessages=0;
